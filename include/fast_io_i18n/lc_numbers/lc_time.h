...
lc_print_reserve_size_time_format_common_impl(basic_lc_time<char_type> const &t, iso8601_timestamp const &tsp,
    ...
    for (char_type const *i{format_str.base}, *end_it{i + format_str.len}; i != end_it; ++i) {
        ...
        case char_literal_v<u8'O', char_type>: {
            ...
            case char_literal_v<u8'p',
                                char_type>: // Glibc's changelog said do not use %Op but the upstream still use this
    ...
    for (char_type const *i{format_str.base}, *end_it{i + format_str.len}; i != end_it; ++i) {
            ...
            case char_literal_v<u8'Y', char_type>: {
                ...
                if (era_result != era_scatter_end) [[likely]] {
                    ...
                    default: {
                        ...
                        if (::fast_io::details::intrinsics::sub_underflow(tsp.year, real_era_year_offset, era_year))
                            [[unlikely]] {
                            if (omit)
        ...
        case char_literal_v<u8'v', char_type>: {
            ...
            if (month == 1u && weekday_of_1st_day_this_year.u > ndays - first_week + 1u &&
                ...
                ::std::uint_least64_t weeknum{static_cast<::std::uint_least64_t>(
                                                  day_of_12_31 + weekday_of_1st_day_last_year - weekday_of_12_31 - 1u) /
                                              ndays};
        ...
        case char_literal_v<u8'W', char_type>: {
            iter = chrono_two_digits_impl<false>(
                iter,
                static_cast<::std::uint_least16_t>(day_of_the_year(tsp) - weekday(tsp) + weekday(tsp.year, 1, 1) - 1u) /
                    7u);
            break;
        }
        case char_literal_v<u8'x', char_type>: {
