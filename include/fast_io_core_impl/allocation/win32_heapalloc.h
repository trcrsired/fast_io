...
#endif
inline void*
win32_heapalloc_common_impl(::std::size_t to_allocate, ::std::uint_least32_t flag) noexcept {
	if (to_allocate == 0) {
		to_allocate = 1;
	}
...
#endif
inline void*
win32_heaprealloc_common_impl(void* addr, ::std::size_t n, ::std::uint_least32_t flag) noexcept {
	if (n == 0) {
		n = 1;
	}
	if (addr == nullptr)
