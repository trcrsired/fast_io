...
enum class scalar_placement : char8_t { none, left, middle, right, internal };

enum class floating_format : char8_t { fixed, general, scientific, decimal, hexfloat };
...
enum class percentage_flag : ::std::uint_least8_t { none, percent, sexratio };
...
inline constexpr scalar_manip_t<::fast_io::details::boolalpha_mani_flags_cache<upper>, bool>
boolalpha(bool b) noexcept {
    return {b};
}
...
print_reserve_size(io_reserve_type_t<char_type, manipulators::scalar_manip_t<flags, T>>) noexcept {
    if constexpr (flags.alphabet) {
        ...
        if constexpr (::std::same_as<::std::remove_cvref_t<T>, bool>)
            ...
            return 7; // u8"nullptr"
    } else if constexpr (::std::same_as<::std::remove_cv_t<T>,
                                        ::fast_io::manipulators::member_function_pointer_holder_t>) {
