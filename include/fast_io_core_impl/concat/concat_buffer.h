...
struct basic_concat_buffer {
	...
	constexpr basic_concat_buffer() noexcept : buffer_begin{stack_buffer},
	...
	constexpr
		...
		if (buffer_begin != stack_buffer) [[unlikely]] {
			::fast_io::details::deallocate_iobuf_space<false, ch_type>(buffer_begin, static_cast<::std::size_t>(buffer_end - buffer_begin));
		}
	}
};
...
basic_concat_buffer_strlike_reserve_cold_impl(::fast_io::details::basic_concat_buffer<char_type>& str, ::std::size_t n, ::std::size_t df) noexcept {
	auto old_buffer_begin_ptr{str.buffer_begin};
	bool onstack{old_buffer_begin_ptr == str.stack_buffer};
	auto newptr{::fast_io::details::allocate_iobuf_space<char_type>(n)};
	::std::size_t const elements{static_cast<::std::size_t>(str.buffer_curr - str.buffer_begin)};
	auto newcurr_ptr{::fast_io::freestanding::non_overlapped_copy_n(old_buffer_begin_ptr, elements, newptr)};
	str.buffer_curr = newcurr_ptr;
	if (!onstack) {
		::fast_io::details::deallocate_iobuf_space<false, char_type>(old_buffer_begin_ptr, df);
	}
