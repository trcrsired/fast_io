...
inline constexpr char_type* print_reserve_define_chain_impl(char_type* p, T t, Args... args) {
	if constexpr (sizeof...(Args) == 0) {
		p = print_reserve_define(::fast_io::io_reserve_type<char_type, ::std::remove_cvref_t<T>>, p, t);
		if constexpr (line) {
			*p = ::fast_io::char_literal_v<u8'\n', char_type>;
			++p;
		}
		return p;
	} else
		return print_reserve_define_chain_impl<line>(print_reserve_define(::fast_io::io_reserve_type<char_type, ::std::remove_cvref_t<T>>, p, t), args...);
}
...
inline constexpr ::std::size_t calculate_scatter_dynamic_reserve_size_with_scatter([[maybe_unused]] T t, Args... args) {
	if constexpr (dynamic_reserve_printable<char_type, T>) {
		::std::size_t res{print_reserve_size(::fast_io::io_reserve_type<char_type, T>, t)};
		if constexpr (sizeof...(Args) == 0)
			return res;
		else
		...
		::std::size_t res{print_scatter_define(::fast_io::io_reserve_type<char_type, ::std::remove_cvref_t<T>>, t).len};
		if constexpr (sizeof...(Args) == 0)
			return res;
		else
			return ::fast_io::details::intrinsics::add_or_overflow_die(res, calculate_scatter_dynamic_reserve_size_with_scatter<char_type>(args...));
...
inline constexpr char_type* print_reserve_define_chain_scatter_impl(char_type* p, T t, Args... args) {
	if constexpr (dynamic_reserve_printable<char_type, T> || reserve_printable<char_type, T>)
		...
		auto sc{print_scatter_define(::fast_io::io_reserve_type<char_type, ::std::remove_cvref_t<T>>, t)};
		p = ::fast_io::freestanding::non_overlapped_copy_n(sc.base, sc.len, p);
	}
	if constexpr (sizeof...(Args) == 0) {
		if constexpr (line) {
			*p = ::fast_io::char_literal_v<u8'\n', char_type>;
...
inline constexpr basic_io_scatter_t<ch_type> print_scatter_define_extract_one(T t) {
	return print_scatter_define(::fast_io::io_reserve_type<ch_type, ::std::remove_cvref_t<T>>, t);
}
...
inline constexpr T basic_general_concat_decay_impl_precise(T& str, Arg arg) {
	::std::size_t precise_size{print_reserve_precise_size(::fast_io::io_reserve_type<ch_type, Arg>, arg)};
	::std::size_t precise_size_with_line{precise_size};
	if constexpr (line)
		++precise_size_with_line;
	constexpr ::std::size_t local_cap{strlike_sso_size(::fast_io::io_strlike_type<ch_type, T>)};
	if (local_cap < precise_size_with_line)
		strlike_reserve(::fast_io::io_strlike_type<ch_type, T>, str, local_cap);
	auto first{strlike_begin(::fast_io::io_strlike_type<ch_type, T>, str)};
	print_reserve_precise_define(::fast_io::io_reserve_type<ch_type, Arg>, first, precise_size, arg);
	auto ptr{first + precise_size};
	if constexpr (line) {
		*ptr = ::fast_io::char_literal_v<u8'\n', ch_type>;
		++ptr;
	}
	strlike_set_curr(::fast_io::io_strlike_type<ch_type, T>, str, ptr);
	return str;
}
...
inline constexpr T basic_general_concat_decay_impl(Args... args) {
	if constexpr (sizeof...(Args) == 0) {
		if constexpr (line) {
			return strlike_construct_single_character_define(::fast_io::io_strlike_type<ch_type, T>, ::fast_io::char_literal_v<u8'\n', ch_type>);
		} else {
			return {};
		}
		...
		if constexpr ((reserve_printable<ch_type, Args> && ...)) {
			if constexpr (sso_buffer_strlike<ch_type, T>) {
				constexpr ::std::size_t local_cap{strlike_sso_size(::fast_io::io_reserve_type<ch_type, T>)};
				...
				if constexpr (not_enough_space && ((sizeof...(Args) == 1) && (precise_reserve_printable<ch_type, Args> && ...))) {
					...
					if constexpr (not_enough_space)
						strlike_reserve(::fast_io::io_strlike_type<ch_type, T>, str, sz_with_line);
					strlike_set_curr(::fast_io::io_strlike_type<ch_type, T>, str,
									 print_reserve_define_chain_impl<line>(strlike_begin(::fast_io::io_strlike_type<ch_type, T>, str), args...));
					return str;
				}
			} else {
				T str;
			...
			if constexpr ((!line) && sizeof...(args) == 1 && (scatter_printable<ch_type, Args> && ...)) {
				...
				return strlike_construct_define(::fast_io::io_strlike_type<ch_type, T>(scatter.base, scatter.base + scatter.len));
			} else {
				...
				strlike_set_curr(::fast_io::io_strlike_type<ch_type, T>, str,
								 print_reserve_define_chain_scatter_impl<line>(strlike_begin(::fast_io::io_strlike_type<ch_type, T>, str), args...));
				return str;
			}
		}
	} else {
		T str;
		auto oref{::fast_io::details::io_strlike_ref(::fast_io::io_alias, str)};
...
inline constexpr void basic_general_concat_decay_ref_impl_precise(T& str, Arg arg) {
	...
	constexpr ::std::size_t local_cap{strlike_sso_size(::fast_io::io_strlike_type<ch_type, T>)};
	if (local_cap < precise_size_with_line)
		strlike_reserve(::fast_io::io_strlike_type<ch_type, T>, str, precise_size_with_line);
	auto first{strlike_begin(::fast_io::io_strlike_type<ch_type, T>, str)};
	print_reserve_precise_define(::fast_io::io_reserve_type<ch_type, Arg>, first, precise_size, arg);
	auto ptr{first + precise_size};
	if constexpr (line) {
	...
	if constexpr (sizeof...(Args) == 0) {
		if constexpr (line) {
			if constexpr (single_character_constructible_strlike<ch_type, T>)
				...
				return strlike_construct_define(::fast_io::io_strlike_type<ch_type, T>, __builtin_addressof(::fast_io::char_literal_v<u8'\n', ch_type>), __builtin_addressof(char_literal_v<u8'\n', ch_type>) + 1);
			}
		} else {
			return {};
		}
		...
		if constexpr (buffer_strlike<ch_type, T>) {
			...
			return strlike_construct_define(::fast_io::io_strlike_type<ch_type, T>, buffer, p);
		} else {
			basic_concat_buffer<ch_type> buffer;
			basic_general_concat_decay_ref_impl<line, ch_type>(buffer, args...);
			return strlike_construct_define(::fast_io::io_strlike_type<ch_type, T>, buffer.buffer_begin, buffer.buffer_curr);
		}
	}
}
