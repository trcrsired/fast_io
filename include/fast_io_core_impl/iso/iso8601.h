...
print_reserve_size(io_reserve_type_t<char_type, basic_timestamp<off_to_epoch>>) noexcept {
...
#define FAST_IO_SCAN_ISO8601_CONTEXT_TOKEN_PHASE(TOK)                                                                  \
    ...
    case scan_timestamp_context_phase::minutes:
        if (auto [itr, ec] = scan_iso8601_context_2_digits_phase(state, begin, end, t.minutes); ec != parse_code::ok)
            [[unlikely]]
            return {itr, ec};
        else {
    ...
    case scan_timestamp_context_phase::seconds:
        if (auto [itr, ec] = scan_iso8601_context_2_digits_phase(state, begin, end, t.seconds); ec != parse_code::ok)
            [[unlikely]]
            return {itr, ec};
        else {
    ...
    case scan_timestamp_context_phase::after_subseconds_timezone_marker: {
        ...
        case comma ? char_literal_v<u8',', char_type>:
            char_literal_v<u8'.', char_type> : if (state.tsp_phase == scan_timestamp_context_phase::timezone_marker) {
                state.tsp_phase = scan_timestamp_context_phase::subseconds;
    ...
    case scan_timestamp_context_phase::timezone_hours:
        if (auto [itr, ec] = scan_iso8601_context_2_digits_phase(state, begin, end, t.timezone); ec != parse_code::ok)
            [[unlikely]]
            return {itr, ec};
        else {
