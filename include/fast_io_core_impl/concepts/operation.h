...
{
	{scan_contiguous_define(::fast_io::io_reserve_type<char_type,T>,begin,end,t)}->::std::same_as<parse_result<char_type const*>>;
};

template<typename char_type, typename T>
concept context_scannable = requires(char_type const* begin, char_type const* end, T t)
{
	requires requires(typename ::std::remove_cvref_t<decltype(scan_context_type(::fast_io::io_reserve_type<char_type,T>))>::type st)
	{
		{scan_context_define(::fast_io::io_reserve_type<char_type,T>,st,begin,end,t)}->::std::same_as<parse_result<char_type const*>>;
		{scan_context_eof_define(::fast_io::io_reserve_type<char_type,T>,st,t)}->::std::same_as<parse_code>;
	};
};
...
{
	{print_reserve_size(::fast_io::io_reserve_type<char_type,::std::remove_cvref_t<T>>)}->::std::convertible_to<::std::size_t>;
	{print_reserve_define(::fast_io::io_reserve_type<char_type,::std::remove_cvref_t<T>>,ptr,t)}->::std::convertible_to<char_type*>;
};

template<typename char_type,typename T>
concept dynamic_reserve_printable=::std::integral<char_type>&&requires(T t,char_type* ptr)
{
	{print_reserve_size(::fast_io::io_reserve_type<char_type,::std::remove_cvref_t<T>>,t)}->::std::convertible_to<::std::size_t>;
	{print_reserve_define(::fast_io::io_reserve_type<char_type,::std::remove_cvref_t<T>>,ptr,t)}->::std::convertible_to<char_type*>;
};
#if 0
template<typename char_type,typename T>
...
{
	{print_context_reserve_size(::fast_io::io_reserve_type<char_type,::std::remove_cvref_t<T>>)}->::std::convertible_to<::std::size_t>;
	requires requires(typename ::std::remove_cvref_t<decltype(print_context_type(::fast_io::io_reserve_type<char_type,T>))>::type st)
	{
	{print_context_define(::fast_io::io_reserve_type<char_type,::std::remove_cvref_t<T>>,ptr,st)};
	}
};
#endif
template<typename char_type,typename T>
...
{
	{print_define_internal_shift(::fast_io::io_reserve_type<char_type,::std::remove_cvref_t<T>>,t)}->::std::same_as<::std::size_t>;
};
...
{
	{print_reserve_precise_size(::fast_io::io_reserve_type<char_type,::std::remove_cvref_t<T>>,t)}->::std::convertible_to<::std::size_t>;
	print_reserve_precise_define(::fast_io::io_reserve_type<char_type,::std::remove_cvref_t<T>>,ptr,n,t);
};
...
{
	{print_reserve_scatters_size(::fast_io::io_reserve_type<char_type,::std::remove_cvref_t<T>>)}->::std::same_as<reserve_scatters_size_result>;
	{print_reserve_scatters_define(::fast_io::io_reserve_type<char_type,::std::remove_cvref_t<T>>,scatters,ptr,t)}->::std::same_as<::fast_io::basic_reserve_scatters_define_result<char_type>>;
};

template<typename char_type,typename T>
concept printable=requires(::fast_io::details::dummy_buffer_output_stream<char_type> out,T t)
{
	print_define(::fast_io::io_reserve_type<char_type,::std::remove_cvref_t<T>>,out,t);
};

template<typename char_type,typename T>
concept scatter_printable=requires(char_type ch,T&& t)
{
	{print_scatter_define(::fast_io::io_reserve_type<char_type,::std::remove_cvref_t<T>>,::fast_io::freestanding::forward<T>(t))}->::std::same_as<basic_io_scatter_t<char_type>>;
};

template<typename T>
concept alias_scannable=requires(T&& t)
{
	scan_alias_define(::fast_io::io_alias,t);
};

template<typename T>
concept alias_printable=requires(T&& t)
{
	print_alias_define(::fast_io::io_alias,::fast_io::freestanding::forward<T>(t));
};

template<typename char_type,typename T>
concept status_io_scan_forwardable=::std::integral<char_type>&&requires(T t)
{
	status_io_scan_forward(::fast_io::io_alias_type<char_type>,t);
};

template<typename char_type,typename T>
concept status_io_print_forwardable=::std::integral<char_type>&&requires(T&& t)
{
	status_io_print_forward(::fast_io::io_alias_type<char_type>,::fast_io::freestanding::forward<T>(t));
};
...
{
	return print_define_internal_shift(::fast_io::io_reserve_type<char_type,::std::remove_cvref_t<value_type>>,begin,para.reference);
}
...
{
	print_reserve_precise_define(::fast_io::io_reserve_type<char_type,::std::remove_cvref_t<value_type>>,begin,n,para.reference);
}
...
concept iterative_scannable = ::std::integral<char_type>&&requires(T& t,char_type const* buffer_curr,char_type const* buffer_end)
{
	...
	{scan_iterative_next_define(::fast_io::io_reserve_type<char_type,::std::remove_cvref_t<T>>,t,buffer_curr,buffer_end)}->::std::same_as<parse_result<char_type const*>>;
	{scan_iterative_eof_define(::fast_io::io_reserve_type<char_type,::std::remove_cvref_t<T>>,t)}->::std::same_as<fast_io::parse_code>;
};
...
{
	{scan_iterative_contiguous_define(::fast_io::io_reserve_type<char_type,::std::remove_cvref_t<T>>,t,buffer_curr,buffer_end)}->::std::same_as<parse_result<char_type const*>>;
};

template<typename char_type,typename T>
concept precise_reserve_scannable = ::std::integral<char_type>&&requires(char_type const* buffer_curr,T t)
{
	{scan_precise_reserve_size(::fast_io::io_reserve_type<char_type,::std::remove_cvref_t<T>>)}->::std::same_as<::std::size_t>;
...
{
	{scan_precise_reserve_define(::fast_io::io_reserve_type<char_type,::std::remove_cvref_t<T>>,buffer_curr,t)}->::std::same_as<void>;
};
