...
using basic_utf_mb_state = ::std::conditional_t<sizeof(char_type) == 4,
												u32utf_mb_state,
												::std::conditional_t<sizeof(char_type) == 2,
																	 u16utf_mb_state,
																	 u8utf_mb_state>>;
...
template <::std::integral char_type>
inline constexpr ::fast_io::encoding_scheme execution_charset_encoding_scheme() noexcept {
	using char_type_no_cvref_t = ::std::remove_cvref_t<char_type>;
	if constexpr (::fast_io::details::is_ebcdic<char_type_no_cvref_t>)
		return ::fast_io::encoding_scheme::utf_ebcdic;
	else {
		if constexpr (::std::same_as<char_type_no_cvref_t, char>) {
			...
			if constexpr (::fast_io::details::execution_charset_is(__GNUC_EXECUTION_CHARSET_NAME, u8"GB18030") || ::fast_io::details::execution_charset_is(__GNUC_EXECUTION_CHARSET_NAME, u8"GBK")) {
				return ::fast_io::encoding_scheme::gb18030;
			} else {
				return ::fast_io::encoding_scheme::utf;
			}
			...
			if constexpr (sizeof(wchar_t) == 1 &&
						  (::fast_io::details::execution_charset_is(__GNUC_WIDE_EXECUTION_CHARSET_NAME, u8"GB18030") || ::fast_io::details::execution_charset_is(__GNUC_WIDE_EXECUTION_CHARSET_NAME, u8"GBK"))) {
				return ::fast_io::encoding_scheme::gb18030;
			} else {
				if constexpr (::fast_io::details::wide_is_none_utf_endian) {
					if constexpr (::fast_io::encoding_scheme::utf == ::fast_io::encoding_scheme::utf_le) {
						return ::fast_io::encoding_scheme::utf_be;
					} else {
						return ::fast_io::encoding_scheme::utf_le;
					}
				}
				return ::fast_io::encoding_scheme::utf;
			}
...
template <::std::integral char_type, ::fast_io::encoding_scheme scheme = execution_charset_encoding_scheme<char_type>()>
...
inline constexpr ::std::size_t get_utf_code_units(char32_t cdpt, T* dst) noexcept {
	if constexpr (scheme == encoding_scheme::utf_ebcdic) {
		return ::fast_io::details::get_utf_ebcdic_code_units(cdpt, dst);
	} else if constexpr (scheme == encoding_scheme::gb18030) {
		return ::fast_io::details::codecvt::gb18030::get_gb18030_code_units(cdpt, dst);
	} else {
		if constexpr (sizeof(T) == sizeof(char32_t)) {
			if constexpr (::fast_io::details::codecvt::is_native_scheme(scheme))
				*dst = static_cast<T>(cdpt);
			else
				*dst = ::fast_io::byte_swap(static_cast<T>(cdpt));
			return 1;
			...
			if (cdpt < 0x10000) {
				...
				if constexpr (::fast_io::details::codecvt::is_native_scheme(scheme)) {
					...
					*dst = ::fast_io::byte_swap(static_cast<T>(0xD7C0 + (cdpt >> 10)));
					dst[1] = ::fast_io::byte_swap(static_cast<T>(0xDC00 + (cdpt & 0x3FF)));
				}
				return 2;
			}
		} else {
...
inline constexpr advance_with_big_table_unchecked_result<T> advance_with_big_table_unchecked(T const* it) noexcept {
	char8_t const* info{::fast_io::details::codecvt::first_unit_info[static_cast<char8_t>(*it)]};
	...
	for (++it; 12 < curr;) {
		...
		curr = ::fast_io::details::codecvt::transitions[curr + ::fast_io::details::codecvt::octet_category[unit]];
...
inline constexpr advance_with_big_table_result<T> advance_with_big_table(T const* first, T const* last) noexcept {
	char8_t const* info{::fast_io::details::codecvt::first_unit_info[static_cast<char8_t>(*first)]};
	char32_t cdpt{static_cast<char32_t>(*info)};  //- From it, get the initial code point value
	::std::int_least32_t curr{info[1]};           //- From it, get the second state
	auto it{first};
	for (++it; 12 < curr;) {
