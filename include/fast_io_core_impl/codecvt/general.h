...
inline constexpr auto general_advance(T* src_first, ::std::size_t sz) noexcept {
	if constexpr (encoding_scheme::utf_ebcdic == encoding)
		return ::fast_io::details::utf_ebcdic_advance(src_first, sz);
	else
		return ::fast_io::details::codecvt::gb18030::gb18030_advance(src_first, sz);
}

template <encoding_scheme encoding, typename T>
inline constexpr ::std::size_t get_general_invalid_code_units(T* dst) noexcept {
	if constexpr (encoding == ::fast_io::encoding_scheme::utf_ebcdic)
		return get_utf_ebcdic_invalid_code_units(dst);
	else if constexpr (encoding == ::fast_io::encoding_scheme::gb18030)
		return ::fast_io::details::codecvt::gb18030::get_gb18030_invalid_code_units(dst);
	else {
		if constexpr (sizeof(T) >= 2) {
			if constexpr (::fast_io::details::codecvt::encoding_is_utf(encoding) &&
						  !::fast_io::details::codecvt::is_native_scheme(encoding)) {
			...
			return ::fast_io::details::codecvt::get_utf8_invalid_code_units(dst);
	}
}

template <::std::integral char_type>
inline constexpr ::fast_io::encoding_scheme get_execution_charset_encoding_scheme(encoding_scheme scheme) noexcept {
	if (scheme != ::fast_io::encoding_scheme::execution_charset)
		return scheme;
	return execution_charset_encoding_scheme<char_type>();
}

template <
	encoding_scheme src_encoding = ::fast_io::encoding_scheme::execution_charset,
	encoding_scheme encoding = ::fast_io::encoding_scheme::execution_charset,
...
inline constexpr code_cvt_result<src_char_type, dest_char_type> general_code_cvt(src_char_type const* src_first, src_char_type const* src_last, dest_char_type* __restrict dst) noexcept {
	if constexpr (src_encoding == ::fast_io::encoding_scheme::execution_charset) {
		return general_code_cvt<get_execution_charset_encoding_scheme<src_char_type>(src_encoding),
								encoding>(src_first, src_last, dst);
	} else if constexpr (encoding == ::fast_io::encoding_scheme::execution_charset) {
		return general_code_cvt<src_encoding, get_execution_charset_encoding_scheme<dest_char_type>(encoding)>(src_first, src_last, dst);
		...
		::fast_io::freestanding::non_overlapped_copy_n(src_first, diff, dst);
		return {src_last, dst + diff};
	} else if constexpr (sizeof(src_char_type) == sizeof(dest_char_type) &&
						 ((src_encoding == ::fast_io::encoding_scheme::utf_le && encoding == ::fast_io::encoding_scheme::utf_be) ||
						  (src_encoding == ::fast_io::encoding_scheme::utf_be && encoding == ::fast_io::encoding_scheme::utf_le))) {
		if constexpr (sizeof(src_char_type) == 1) {
			return general_code_cvt<encoding_scheme::utf, ::fast_io::encoding_scheme::utf>(src_first, src_last, dst);
		} else {
			for (; src_first != src_last; ++src_first)
				*dst = ::fast_io::byte_swap(*src_first);
			return {src_first, dst};
		}
	} else if constexpr (sizeof(src_char_type) == 4) {
		static_assert(src_encoding == ::fast_io::encoding_scheme::utf_be || src_encoding == ::fast_io::encoding_scheme::utf_le);
		for (; src_first != src_last; ++src_first)
			dst += ::fast_io::details::codecvt::get_utf_code_units<encoding>(static_cast<char32_t>(*src_first), dst);
		return {src_last, dst};
		...
		*/
		for (; src_first != src_last; ++src_first) {
			...
			if constexpr (!::fast_io::details::codecvt::is_native_scheme(src_encoding))
				code = ::fast_io::byte_swap(code);
			if (::fast_io::details::codecvt::is_utf16_surrogate(code)) [[unlikely]] {
				if (::fast_io::details::codecvt::is_utf16_high_surrogate(code)) {
					if (src_first + 1 == src_last) {
						break;
					}
					char16_t code1{static_cast<char16_t>(*++src_first)};
					if constexpr (!::fast_io::details::codecvt::is_native_scheme(src_encoding))
						code1 = ::fast_io::byte_swap(code1);
					if (::fast_io::details::codecvt::is_utf16_low_surrogate(code1)) {
						if constexpr (sizeof(dest_char_type) == 4) {
							*dst = ::fast_io::details::codecvt::utf16_surrogate_to_utf32(code, code1);
							++dst;
						} else
							dst += ::fast_io::details::codecvt::get_utf_code_units<encoding>(
								::fast_io::details::codecvt::utf16_surrogate_to_utf32(code, code1), dst);
						continue;
					}
				}
				if constexpr (sizeof(dest_char_type) == 4) {
					if constexpr (::fast_io::details::codecvt::is_native_scheme(encoding))
				...
				if constexpr (sizeof(dest_char_type) == 4) {
					if constexpr (src_encoding == encoding)
						...
						*dst = ::fast_io::byte_swap(static_cast<dest_char_type>(code));
					++dst;
				} else
					dst += ::fast_io::details::codecvt::get_utf_code_units<encoding>(code, dst);
			}
		}
		return {src_first, dst};
	} else {
		...
		if constexpr (src_encoding != ::fast_io::encoding_scheme::utf_ebcdic &&
					  encoding != ::fast_io::encoding_scheme::utf_ebcdic &&
					  1 == sizeof(src_char_type) &&
					  (1 == sizeof(dest_char_type) || ::fast_io::details::codecvt::encoding_is_utf(encoding))) {
			if (!__builtin_is_constant_evaluated()) {
				...
				while (m128i_size < static_cast<::std::size_t>(src_last - src_first)) {
					if (static_cast<char8_t>(*src_first) < 0x80) {
						auto [new_src, new_dst] = ::fast_io::details::codecvt::convert_ascii_with_sse(src_first, dst);
						src_first = new_src;
						dst = new_dst;
					} else {
						if constexpr (src_encoding == ::fast_io::encoding_scheme::gb18030) {
							auto [code, adv] = ::fast_io::details::codecvt::gb18030::gb18030_advance_unchecked(src_first);
							src_first += adv;
							if constexpr (sizeof(dest_char_type) == 4) {
								*dst = code;
								++dst;
							...
							auto [src, code] = ::fast_io::details::codecvt::advance_with_big_table_unchecked(src_first);
							src_first = src;
							if constexpr (sizeof(dest_char_type) == 4) {
								*dst = code;
								++dst;
		...
		for (; src_first != src_last;)
			if constexpr (src_encoding == ::fast_io::encoding_scheme::utf_ebcdic) {
				...
				if (static_cast<char8_t>(*src_first) < 0x80) {
					if constexpr (encoding_scheme::utf_ebcdic == encoding)
						*dst = static_cast<dest_char_type>(::fast_io::details::bm_i8_to_ebcdic[*src_first]);
					else
						*dst = static_cast<char8_t>(*src_first);
					if constexpr (sizeof(dest_char_type) != 1 &&
								  ::fast_io::details::codecvt::encoding_is_utf(encoding) &&
								  !::fast_io::details::codecvt::is_native_scheme(encoding))
						*dst = ::fast_io::byte_swap(*dst);
					++src_first;
					++dst;
				} else {
					if constexpr (src_encoding != ::fast_io::encoding_scheme::utf) {
						auto [code, adv] = general_advance<src_encoding>(src_first, src_last - src_first);
						if (adv == static_cast<char8_t>(-1))
							break;
						src_first += adv;
						if constexpr (sizeof(dest_char_type) == 4) {
							*dst = code;
							if constexpr (::fast_io::details::codecvt::encoding_is_utf(encoding) && !::fast_io::details::codecvt::is_native_scheme(encoding))
								*dst = ::fast_io::byte_swap(*dst);
							++dst;
						} else {
							dst += ::fast_io::details::codecvt::get_utf_code_units<encoding>(code, dst);
						...
						auto [failed, src, code] = ::fast_io::details::codecvt::advance_with_big_table(src_first, src_last);
						if (failed)
							break;
						src_first = src;
						if constexpr (sizeof(dest_char_type) == 4) {
...
template <
	encoding_scheme src_encoding = ::fast_io::encoding_scheme::execution_charset,
	encoding_scheme encoding = ::fast_io::encoding_scheme::execution_charset, typename state_type,
	::std::integral src_char_type, ::std::integral dest_char_type>
	requires(sizeof(src_char_type) <= 4 && sizeof(dest_char_type) <= 4)
inline constexpr dest_char_type* general_code_cvt(state_type& __restrict state, src_char_type const* src_first, src_char_type const* src_last, dest_char_type* __restrict dst) noexcept {
	if constexpr (src_encoding == ::fast_io::encoding_scheme::execution_charset) {
		return general_code_cvt<get_execution_charset_encoding_scheme<src_char_type>(src_encoding), encoding>(state, src_first, src_last, dst);
	} else if constexpr (encoding == ::fast_io::encoding_scheme::execution_charset) {
		return general_code_cvt<src_encoding, get_execution_charset_encoding_scheme<dest_char_type>(encoding)>(state, src_first, src_last, dst);
	} else if constexpr (sizeof(src_char_type) == 4) {
		static_assert(src_encoding == ::fast_io::encoding_scheme::utf);
		...
		if (state.state) {
			...
			if constexpr (!::fast_io::details::codecvt::is_native_scheme(src_encoding))
				low = ::fast_io::byte_swap(low);
			char16_t v{static_cast<char16_t>(*src_first)};
			if constexpr (!::fast_io::details::codecvt::is_native_scheme(src_encoding))
				v = ::fast_io::byte_swap(v);
			if (::fast_io::details::codecvt::is_utf16_low_surrogate(v)) {
				if constexpr (sizeof(dest_char_type) == 4) {
					*dst = ::fast_io::details::codecvt::utf16_surrogate_to_utf32(low, v);
					if constexpr (!::fast_io::details::codecvt::is_native_scheme(encoding))
						*dst = ::fast_io::byte_swap(*dst);
					++dst;
				} else
					dst += ::fast_io::details::codecvt::get_utf_code_units<encoding>(
						::fast_io::details::codecvt::utf16_surrogate_to_utf32(low, v), dst);
				++src_first;
			} else [[unlikely]] {
				if constexpr (sizeof(dest_char_type) == 4) {
					if constexpr (::fast_io::details::codecvt::is_native_scheme(encoding))
		...
		if (state_size) {
			...
			::fast_io::freestanding::non_overlapped_copy_n(state.bytes, state_bytesm1, bytes);
			if (src_diff < remain_unsolved)
				remain_unsolved = src_diff;
			::fast_io::freestanding::non_overlapped_copy_n(src_first, remain_unsolved, bytes + state_size);
			::std::size_t total_bytes{state_size + remain_unsolved};
			if constexpr (src_encoding == ::fast_io::encoding_scheme::utf) {
				auto [failed, bytes_src, code] = ::fast_io::details::codecvt::advance_with_big_table(bytes, bytes + total_bytes);
				if (failed) {
					::fast_io::freestanding::non_overlapped_copy_n(bytes, state_bytesm1, state.bytes);
		...
		::fast_io::freestanding::non_overlapped_copy_n(new_src, diff, state.bytes);
		return new_dst;
	} else
		return dst;
}
...
template <
	encoding_scheme src_scheme = ::fast_io::encoding_scheme::execution_charset,
	encoding_scheme dst_scheme = ::fast_io::encoding_scheme::execution_charset,
	typename T>
...
inline constexpr ::std::size_t print_reserve_size(
	...
	return ::fast_io::details::cal_full_reserve_size<sizeof(src_char_type), sizeof(dst_char_type)>(v.reference.len);
}
