...
inline constexpr ::std::size_t deco_reserve_size(io_reserve_type_t<to_char_type,
	...
	return ::fast_io::details::cal_decorated_reserve_size<sizeof(from_char_type), sizeof(to_char_type)>(from_size);
}
...
inline constexpr to_iter deco_reserve_define(io_reserve_type_t<::std::iter_value_t<to_iter>,
	...
	if constexpr (::std::is_pointer_v<from_iter> && ::std::is_pointer_v<to_iter>)
		return ::fast_io::details::codecvt::general_code_cvt<from_scheme, to_scheme>(conv.state, src_first, src_last, dst);
	else if constexpr (::std::is_pointer_v<to_iter>)
		return ::fast_io::details::codecvt::general_code_cvt<from_scheme, to_scheme>(conv.state,
																		  ::std::to_address(src_first),
																		  ::std::to_address(src_last),
																		  dst);
	else {
		return dst + (::fast_io::details::codecvt::general_code_cvt<from_scheme, to_scheme>(conv.state,
																				 ::std::to_address(src_first),
																				 ::std::to_address(src_last),
																				 ::std::to_address(dst)) -
					  ::std::to_address(dst));
