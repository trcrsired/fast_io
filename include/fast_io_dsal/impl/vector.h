...
template <typename allocator>
inline void zero_init_grow_to_size_aligned_impl(vector_model *m, ::std::size_t alignment,
                                                ::std::size_t newcap) noexcept {
    auto begin_ptr{m->begin_ptr};
    ::std::size_t const old_size{static_cast<::std::size_t>(m->curr_ptr - begin_ptr)};
    if constexpr (allocator::has_reallocate_aligned) {
...
class
  ...
  public:
    ...
    constexpr vector(size_type n,
                     value_type const &value) noexcept(::std::is_nothrow_copy_constructible_v<value_type>) {
        auto begin_ptr{typed_allocator_type::allocate(n)};
  ...
  private:
    ...
    assign_common_impl(InputIt first, InputIt last) noexcept(::std::is_nothrow_copy_constructible_v<value_type>) {
