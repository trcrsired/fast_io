...
inline constexpr ::fast_io::containers::array<field_number, 16> NxBPOraw{{
	field_number_from_u64(0x0, 0x0, 0x0, 0x0),
	field_number_from_u64(0x5812631a5cf5d3ed, 0x14def9dea2f79cd6, 0x0, 0x1000000000000000),
	field_number_from_u64(0xb024c634b9eba7da, 0x29bdf3bd45ef39ac, 0x0, 0x2000000000000000),
	field_number_from_u64(0x837294f16e17bc7, 0x3e9ced9be8e6d683, 0x0, 0x3000000000000000),
	field_number_from_u64(0x60498c6973d74fb4, 0x537be77a8bde7359, 0x0, 0x4000000000000000),
	field_number_from_u64(0xb85bef83d0cd23a1, 0x685ae1592ed6102f, 0x0, 0x5000000000000000),
	field_number_from_u64(0x106e529e2dc2f78e, 0x7d39db37d1cdad06, 0x0, 0x6000000000000000),
	field_number_from_u64(0x6880b5b88ab8cb7b, 0x9218d51674c549dc, 0x0, 0x7000000000000000),
	field_number_from_u64(0xc09318d2e7ae9f68, 0xa6f7cef517bce6b2, 0x0, 0x8000000000000000),
	field_number_from_u64(0x18a57bed44a47355, 0xbbd6c8d3bab48389, 0x0, 0x9000000000000000),
	field_number_from_u64(0x70b7df07a19a4742, 0xd0b5c2b25dac205f, 0x0, 0xa000000000000000),
	field_number_from_u64(0xc8ca4221fe901b2f, 0xe594bc9100a3bd35, 0x0, 0xb000000000000000),
	field_number_from_u64(0x20dca53c5b85ef1c, 0xfa73b66fa39b5a0c, 0x0, 0xc000000000000000),
	field_number_from_u64(0x78ef0856b87bc309, 0xf52b04e4692f6e2, 0x1, 0xd000000000000000),
	field_number_from_u64(0xd1016b71157196f6, 0x2431aa2ce98a93b8, 0x1, 0xe000000000000000),
	field_number_from_u64(0x2913ce8b72676ae3, 0x3910a40b8c82308f, 0x1, 0xf000000000000000),
}};
...
X is any indexable view of the field_number limb width.
*/
template <typename X>
inline constexpr void eco_reduce_hi_word(field_number &y, field_number::value_type b, X const &x) noexcept
{
	using unsigned_type = field_number::value_type;
	constexpr unsigned_type zero{};
	...
	if constexpr (::std::same_as<unsigned_type, ::std::uint_least32_t>)
	{
		...
		{
			y.index_unchecked(i) = ::fast_io::intrinsics::subc(x.index_unchecked(i), p[i], borrow, borrow);
		}
		y.index_unchecked(6) = ::fast_io::intrinsics::subc(x.index_unchecked(6), zero, borrow, borrow);
		y.index_unchecked(7) = ::fast_io::intrinsics::subc(x.index_unchecked(7), zero, borrow, borrow);
	...
	bool borrow{};
	std::uint_least64_t const x0{static_cast<std::uint_least64_t>(x.index_unchecked(0))};
	std::uint_least64_t const x1{static_cast<std::uint_least64_t>(x.index_unchecked(1))};
	std::uint_least64_t const x2{static_cast<std::uint_least64_t>(x.index_unchecked(2))};
	std::uint_least64_t const x3{static_cast<std::uint_least64_t>(x.index_unchecked(3))};
...
inline constexpr void eco_reduce_hi_word_inplace(u512 &t) noexcept
{
	...
	if constexpr (::std::same_as<unsigned_type, ::std::uint_least32_t>)
	{
		...
		unsigned_type const b{t.index_unchecked(off + n)};
		unsigned_type lo[5] FAST_IO_INDETERMINATE;
		unsigned_type hi[5] FAST_IO_INDETERMINATE;
		for (::std::size_t i{}; i != 5; ++i)
		{
		...
		for (::std::size_t i{}; i != 6; ++i)
		{
			t.index_unchecked(off + i) = ::fast_io::intrinsics::subc(t.index_unchecked(off + i), p[i], borrow, borrow);
		}
		for (::std::size_t i{6}; i != n; ++i)
		{
			t.index_unchecked(off + i) = ::fast_io::intrinsics::subc(t.index_unchecked(off + i), zero, borrow, borrow);
		}
		...
		for (::std::size_t i{}; i != n; ++i)
		{
			t.index_unchecked(off + i) = ::fast_io::intrinsics::addc(t.index_unchecked(off + i), mask & bpo[i], carry, carry);
		}
		return;
	}
	else
	...
	std::uint_least64_t const b{static_cast<std::uint_least64_t>(t.index_unchecked(off + 4))};
	std::uint_least64_t t1;
	...
	bool borrow{};
	t.index_unchecked(off) = static_cast<unsigned_type>(::fast_io::intrinsics::subc(static_cast<std::uint_least64_t>(t.index_unchecked(off)), t0, false, borrow));
	t.index_unchecked(off + 1) = static_cast<unsigned_type>(::fast_io::intrinsics::subc(static_cast<std::uint_least64_t>(t.index_unchecked(off + 1)), t1, borrow, borrow));
	t.index_unchecked(off + 2) = static_cast<unsigned_type>(::fast_io::intrinsics::subc(static_cast<std::uint_least64_t>(t.index_unchecked(off + 2)), t2, borrow, borrow));
	t.index_unchecked(off + 3) = static_cast<unsigned_type>(::fast_io::intrinsics::subc(static_cast<std::uint_least64_t>(t.index_unchecked(off + 3)), t3, borrow, borrow));

	/* Add BPO if there is a borrow */
	std::uint_least64_t mask{::fast_io::intrinsics::subc(zero64, zero64, borrow, borrow)};
	t.index_unchecked(off) = static_cast<unsigned_type>(::fast_io::intrinsics::addc(static_cast<std::uint_least64_t>(t.index_unchecked(off)), mask & static_cast<std::uint_least64_t>(0x5812631A5CF5D3EDULL), false, carry));
	t.index_unchecked(off + 1) = static_cast<unsigned_type>(::fast_io::intrinsics::addc(static_cast<std::uint_least64_t>(t.index_unchecked(off + 1)), mask & static_cast<std::uint_least64_t>(0x14DEF9DEA2F79CD6ULL), carry, carry));
	t.index_unchecked(off + 2) = static_cast<unsigned_type>(::fast_io::intrinsics::addc(static_cast<std::uint_least64_t>(t.index_unchecked(off + 2)), zero64, carry, carry));
	t.index_unchecked(off + 3) = static_cast<unsigned_type>(::fast_io::intrinsics::addc(static_cast<std::uint_least64_t>(t.index_unchecked(off + 3)), mask & static_cast<std::uint_least64_t>(0x1000000000000000ULL), carry, carry));
	}
}
...
inline constexpr void eco_mul_reduce(field_number &z, field_number const &x, field_number const &y) noexcept
{
	...
	if constexpr (::std::same_as<field_number::value_type, ::std::uint_least32_t>)
	{
		...
		eco_reduce_hi_word(z, t.index_unchecked(8), t);
	}
	else
	{
		eco_reduce_hi_word_inplace<3>(t);
...
{
	field_number::value_type const c{addition_u256(z, x, y)};
	eco_reduce_hi_word(z, c, z);
}

inline constexpr field_number::value_type bytes_to_limb_little_endian(std::byte const *p) noexcept
{
	field_number::value_type v FAST_IO_INDETERMINATE;
...
inline constexpr void limb_to_bytes_little_endian(std::byte *p, field_number::value_type v) noexcept
{
	if constexpr (::std::endian::native == ::std::endian::big)
	{
		v = ::fast_io::byte_swap(v);
...
inline constexpr void eco_digest_to_words(field_number &y, std::byte const *md) noexcept
{
	...
	{
		t.index_unchecked(i) = bytes_to_limb_little_endian(md + i * sizeof(field_number::value_type));
	}
	if constexpr (::std::same_as<field_number::value_type, ::std::uint_least32_t>)
	{
		eco_reduce_hi_word_inplace<7>(t);
		...
		eco_reduce_hi_word_inplace<2>(t);
		eco_reduce_hi_word_inplace<1>(t);
		eco_reduce_hi_word(y, t.index_unchecked(8), t);
	}
	else
	{
		eco_reduce_hi_word_inplace<3>(t);
