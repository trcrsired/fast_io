...
int main() {
    ...
    constexpr char const *files[]{"aa_DJ",
                                  "aa_ER",
                                  "aa_ER@saaho",
                                  "aa_ET",
                                  "af_ZA",
    ...
    for (std::uint8_t i{}; i < 50; ++i) {
        fast_io::iso8601_timestamp ts{.year = y(gen),
        ...
        for (auto fnm : files) {
            ...
            auto test = [&](char8_t i) {
                std::size_t reserved{::fast_io::details::lc_print_reserve_size_time_format_common_impl(
                    loc.loc.u8all->time, ts, test_format)};
                u8s.resize(reserved);
                auto itr = ::fast_io::details::lc_print_reserve_define_time_fmt_common_impl(
                    loc.loc.u8all->time, u8s.data(), ts, test_format);
