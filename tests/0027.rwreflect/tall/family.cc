...
extern "C" ::std::byte const *fakewrite(::std::byte const *first, ::std::byte const *last) noexcept;

inline constexpr foo output_stream_ref_define(foo f) noexcept {
    return f;
}

inline constexpr ::std::byte const *write_some_bytes_overflow_define(foo, ::std::byte const *first,
                                                                     ::std::byte const *last) {
...
int main() {
    ...
    if constexpr (test_char_put) {
        ...
        if constexpr (use_fake) {
            fakewrite(reinterpret_cast<::std::byte const *>(buffer), reinterpret_cast<::std::byte const *>(buffer + 5));
        } else {
            ::fast_io::operations::write_some(f, buffer, buffer + 5);
        }
    }
