...
{
	auto vec{::gentest()};
	::fast_io::timer t(u8"str_ranked_btree_map");
	::fast_io::str_ranked_btree_map<::std::size_t> bmap;
	{
	...
	}
	{
		::std::size_t total_mapped{};
		{
			...
			for (::std::size_t i{}; i != bmap.size(); ++i)
			{
				total_mapped += bmap.nth_element(i)->mapped();
			}
		}
		::fast_io::io::perrln("total_mapped=", total_mapped);
	}
}
