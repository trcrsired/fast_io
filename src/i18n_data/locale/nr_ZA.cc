#include "../localedef.h"

namespace fast_io_i18n {
namespace {

inline constexpr std::size_t numeric_grouping_storage[]{3};

inline constexpr lc_all lc_all_global{
    .identification =
        {.name = tsc("nr_ZA"),
         .encoding = tsc(FAST_IO_LOCALE_ENCODING),
         .title = tsc("Southern Ndebele locale for South Africa"),
         .source = tsc("Zuza Software Foundation (Translate.org.za)\t\t;\t\tfast_io"),
         .address =
             tsc("PO Box 28364, Sunnyside, 0132, South "
                 "Africa\t\t;\t\thttps://gitee.com/qabeowjbtkwb/fast_io\t\t;\t\thttps://github.com/cppfastio/fast_io"),
         .contact = tsc("Dwayne Bailey\t\t;\t\tfast_io"),
         .email = tsc("dwayne@translate.org.za;euloanty@live.com"),
         .tel = tsc(""),
         .fax = tsc(""),
         .language = tsc("South Ndebele"),
         .territory = tsc("South Africa"),
         .revision = tsc("0.3"),
         .date = tsc("2005-10-13")},
    .monetary = {.int_curr_symbol = tsc("ZAR "),
                 .currency_symbol = tsc("R"),
                 .mon_decimal_point = tsc("."),
                 .mon_thousands_sep = tsc(","),
                 .mon_grouping = {numeric_grouping_storage, 1},
                 .positive_sign = tsc(""),
                 .negative_sign = tsc("-"),
                 .int_frac_digits = 2,
                 .frac_digits = 2,
                 .p_cs_precedes = 1,
                 .p_sep_by_space = 0,
                 .n_cs_precedes = 1,
                 .n_sep_by_space = 0,
                 .p_sign_posn = 1,
                 .n_sign_posn = 1},
    .numeric = {.decimal_point = tsc("."), .thousands_sep = tsc(","), .grouping = {numeric_grouping_storage, 1}},
    .time = {.abday = {tsc("Son"), tsc("Mvu"), tsc("Bil"), tsc("Tha"), tsc("Ne"), tsc("Hla"), tsc("Gqi")},
             .day = {tsc("uSonto"), tsc("uMvulo"), tsc("uLesibili"), tsc("lesithathu"), tsc("uLesine"),
                     tsc("ngoLesihlanu"), tsc("umGqibelo")},
             .abmon = {tsc("Jan"), tsc("Feb"), tsc("Mat"), tsc("Apr"), tsc("Mey"), tsc("Jun"), tsc("Jul"), tsc("Arh"),
                       tsc("Sep"), tsc("Okt"), tsc("Usi"), tsc("Dis")},
             .mon = {tsc("Janabari"), tsc("uFeberbari"), tsc("uMatjhi"), tsc("u-Apreli"), tsc("Meyi"), tsc("Juni"),
                     tsc("Julayi"), tsc("Arhostosi"), tsc("Septemba"), tsc("Oktoba"), tsc("Usinyikhaba"),
                     tsc("Disemba")},
             .d_t_fmt = tsc("%a %-e %b %Y %T %Z"),
             .d_fmt = tsc("%d//%m//%Y"),
             .t_fmt = tsc("%T"),
             .t_fmt_ampm = tsc(""),
             .date_fmt = tsc("%a %b %-e %H:%M:%S %Z %Y"),
             .am_pm = {tsc(""), tsc("")},
             .week = {7, 19971130, 1}},
    .messages = {.yesexpr = tsc("^[+1yY]"), .noexpr = tsc("^[-0nN]")},
    .paper = {.width = 210, .height = 297},
    .telephone = {.tel_int_fmt = tsc("+%c %a %l"),
                  .tel_dom_fmt = tsc("(%A) %l"),
                  .int_select = tsc("00"),
                  .int_prefix = tsc("27")},
    .name = {.name_fmt = tsc("%d%t%g%t%m%t%f")},
    .address = {.postal_fmt = tsc("%f%N%a%N%d%N%b%N%s %h %e %r%N%z %T%N%c%N"),
                .country_name = tsc("iSewula Afrika"),
                .country_post = tsc("ZA"),
                .country_ab2 = tsc("ZA"),
                .country_ab3 = tsc("ZAF"),
                .country_num = 710,
                .country_car = tsc("ZA"),
                .lang_name = tsc("isiNdebele"),
                .lang_ab = tsc("nr"),
                .lang_term = tsc("nbl"),
                .lang_lib = tsc("nbl")},
    .measurement = {.measurement = 1}};

inline constexpr wlc_all wlc_all_global{
    .identification =
        {.name = tsc(L"nr_ZA"),
         .encoding = tsc(FAST_IO_LOCALE_LENCODING),
         .title = tsc(L"Southern Ndebele locale for South Africa"),
         .source = tsc(L"Zuza Software Foundation (Translate.org.za)\t\t;\t\tfast_io"),
         .address =
             tsc(L"PO Box 28364, Sunnyside, 0132, South "
                 L"Africa\t\t;\t\thttps://gitee.com/qabeowjbtkwb/fast_io\t\t;\t\thttps://github.com/cppfastio/fast_io"),
         .contact = tsc(L"Dwayne Bailey\t\t;\t\tfast_io"),
         .email = tsc(L"dwayne@translate.org.za;euloanty@live.com"),
         .tel = tsc(L""),
         .fax = tsc(L""),
         .language = tsc(L"South Ndebele"),
         .territory = tsc(L"South Africa"),
         .revision = tsc(L"0.3"),
         .date = tsc(L"2005-10-13")},
    .monetary = {.int_curr_symbol = tsc(L"ZAR "),
                 .currency_symbol = tsc(L"R"),
                 .mon_decimal_point = tsc(L"."),
                 .mon_thousands_sep = tsc(L","),
                 .mon_grouping = {numeric_grouping_storage, 1},
                 .positive_sign = tsc(L""),
                 .negative_sign = tsc(L"-"),
                 .int_frac_digits = 2,
                 .frac_digits = 2,
                 .p_cs_precedes = 1,
                 .p_sep_by_space = 0,
                 .n_cs_precedes = 1,
                 .n_sep_by_space = 0,
                 .p_sign_posn = 1,
                 .n_sign_posn = 1},
    .numeric = {.decimal_point = tsc(L"."), .thousands_sep = tsc(L","), .grouping = {numeric_grouping_storage, 1}},
    .time = {.abday = {tsc(L"Son"), tsc(L"Mvu"), tsc(L"Bil"), tsc(L"Tha"), tsc(L"Ne"), tsc(L"Hla"), tsc(L"Gqi")},
             .day = {tsc(L"uSonto"), tsc(L"uMvulo"), tsc(L"uLesibili"), tsc(L"lesithathu"), tsc(L"uLesine"),
                     tsc(L"ngoLesihlanu"), tsc(L"umGqibelo")},
             .abmon = {tsc(L"Jan"), tsc(L"Feb"), tsc(L"Mat"), tsc(L"Apr"), tsc(L"Mey"), tsc(L"Jun"), tsc(L"Jul"),
                       tsc(L"Arh"), tsc(L"Sep"), tsc(L"Okt"), tsc(L"Usi"), tsc(L"Dis")},
             .mon = {tsc(L"Janabari"), tsc(L"uFeberbari"), tsc(L"uMatjhi"), tsc(L"u-Apreli"), tsc(L"Meyi"),
                     tsc(L"Juni"), tsc(L"Julayi"), tsc(L"Arhostosi"), tsc(L"Septemba"), tsc(L"Oktoba"),
                     tsc(L"Usinyikhaba"), tsc(L"Disemba")},
             .d_t_fmt = tsc(L"%a %-e %b %Y %T %Z"),
             .d_fmt = tsc(L"%d//%m//%Y"),
             .t_fmt = tsc(L"%T"),
             .t_fmt_ampm = tsc(L""),
             .date_fmt = tsc(L"%a %b %-e %H:%M:%S %Z %Y"),
             .am_pm = {tsc(L""), tsc(L"")},
             .week = {7, 19971130, 1}},
    .messages = {.yesexpr = tsc(L"^[+1yY]"), .noexpr = tsc(L"^[-0nN]")},
    .paper = {.width = 210, .height = 297},
    .telephone = {.tel_int_fmt = tsc(L"+%c %a %l"),
                  .tel_dom_fmt = tsc(L"(%A) %l"),
                  .int_select = tsc(L"00"),
                  .int_prefix = tsc(L"27")},
    .name = {.name_fmt = tsc(L"%d%t%g%t%m%t%f")},
    .address = {.postal_fmt = tsc(L"%f%N%a%N%d%N%b%N%s %h %e %r%N%z %T%N%c%N"),
                .country_name = tsc(L"iSewula Afrika"),
                .country_post = tsc(L"ZA"),
                .country_ab2 = tsc(L"ZA"),
                .country_ab3 = tsc(L"ZAF"),
                .country_num = 710,
                .country_car = tsc(L"ZA"),
                .lang_name = tsc(L"isiNdebele"),
                .lang_ab = tsc(L"nr"),
                .lang_term = tsc(L"nbl"),
                .lang_lib = tsc(L"nbl")},
    .measurement = {.measurement = 1}};

inline constexpr u8lc_all u8lc_all_global{
    .identification =
        {.name = tsc(u8"nr_ZA"),
         .encoding = tsc(FAST_IO_LOCALE_u8ENCODING),
         .title = tsc(u8"Southern Ndebele locale for South Africa"),
         .source = tsc(u8"Zuza Software Foundation (Translate.org.za)\t\t;\t\tfast_io"),
         .address = tsc(
             u8"PO Box 28364, Sunnyside, 0132, South "
             u8"Africa\t\t;\t\thttps://gitee.com/qabeowjbtkwb/fast_io\t\t;\t\thttps://github.com/cppfastio/fast_io"),
         .contact = tsc(u8"Dwayne Bailey\t\t;\t\tfast_io"),
         .email = tsc(u8"dwayne@translate.org.za;euloanty@live.com"),
         .tel = tsc(u8""),
         .fax = tsc(u8""),
         .language = tsc(u8"South Ndebele"),
         .territory = tsc(u8"South Africa"),
         .revision = tsc(u8"0.3"),
         .date = tsc(u8"2005-10-13")},
    .monetary = {.int_curr_symbol = tsc(u8"ZAR "),
                 .currency_symbol = tsc(u8"R"),
                 .mon_decimal_point = tsc(u8"."),
                 .mon_thousands_sep = tsc(u8","),
                 .mon_grouping = {numeric_grouping_storage, 1},
                 .positive_sign = tsc(u8""),
                 .negative_sign = tsc(u8"-"),
                 .int_frac_digits = 2,
                 .frac_digits = 2,
                 .p_cs_precedes = 1,
                 .p_sep_by_space = 0,
                 .n_cs_precedes = 1,
                 .n_sep_by_space = 0,
                 .p_sign_posn = 1,
                 .n_sign_posn = 1},
    .numeric = {.decimal_point = tsc(u8"."), .thousands_sep = tsc(u8","), .grouping = {numeric_grouping_storage, 1}},
    .time = {.abday = {tsc(u8"Son"), tsc(u8"Mvu"), tsc(u8"Bil"), tsc(u8"Tha"), tsc(u8"Ne"), tsc(u8"Hla"), tsc(u8"Gqi")},
             .day = {tsc(u8"uSonto"), tsc(u8"uMvulo"), tsc(u8"uLesibili"), tsc(u8"lesithathu"), tsc(u8"uLesine"),
                     tsc(u8"ngoLesihlanu"), tsc(u8"umGqibelo")},
             .abmon = {tsc(u8"Jan"), tsc(u8"Feb"), tsc(u8"Mat"), tsc(u8"Apr"), tsc(u8"Mey"), tsc(u8"Jun"), tsc(u8"Jul"),
                       tsc(u8"Arh"), tsc(u8"Sep"), tsc(u8"Okt"), tsc(u8"Usi"), tsc(u8"Dis")},
             .mon = {tsc(u8"Janabari"), tsc(u8"uFeberbari"), tsc(u8"uMatjhi"), tsc(u8"u-Apreli"), tsc(u8"Meyi"),
                     tsc(u8"Juni"), tsc(u8"Julayi"), tsc(u8"Arhostosi"), tsc(u8"Septemba"), tsc(u8"Oktoba"),
                     tsc(u8"Usinyikhaba"), tsc(u8"Disemba")},
             .d_t_fmt = tsc(u8"%a %-e %b %Y %T %Z"),
             .d_fmt = tsc(u8"%d//%m//%Y"),
             .t_fmt = tsc(u8"%T"),
             .t_fmt_ampm = tsc(u8""),
             .date_fmt = tsc(u8"%a %b %-e %H:%M:%S %Z %Y"),
             .am_pm = {tsc(u8""), tsc(u8"")},
             .week = {7, 19971130, 1}},
    .messages = {.yesexpr = tsc(u8"^[+1yY]"), .noexpr = tsc(u8"^[-0nN]")},
    .paper = {.width = 210, .height = 297},
    .telephone = {.tel_int_fmt = tsc(u8"+%c %a %l"),
                  .tel_dom_fmt = tsc(u8"(%A) %l"),
                  .int_select = tsc(u8"00"),
                  .int_prefix = tsc(u8"27")},
    .name = {.name_fmt = tsc(u8"%d%t%g%t%m%t%f")},
    .address = {.postal_fmt = tsc(u8"%f%N%a%N%d%N%b%N%s %h %e %r%N%z %T%N%c%N"),
                .country_name = tsc(u8"iSewula Afrika"),
                .country_post = tsc(u8"ZA"),
                .country_ab2 = tsc(u8"ZA"),
                .country_ab3 = tsc(u8"ZAF"),
                .country_num = 710,
                .country_car = tsc(u8"ZA"),
                .lang_name = tsc(u8"isiNdebele"),
                .lang_ab = tsc(u8"nr"),
                .lang_term = tsc(u8"nbl"),
                .lang_lib = tsc(u8"nbl")},
    .measurement = {.measurement = 1}};

inline constexpr u16lc_all u16lc_all_global{
    .identification =
        {.name = tsc(u"nr_ZA"),
         .encoding = tsc(FAST_IO_LOCALE_uENCODING),
         .title = tsc(u"Southern Ndebele locale for South Africa"),
         .source = tsc(u"Zuza Software Foundation (Translate.org.za)\t\t;\t\tfast_io"),
         .address =
             tsc(u"PO Box 28364, Sunnyside, 0132, South "
                 u"Africa\t\t;\t\thttps://gitee.com/qabeowjbtkwb/fast_io\t\t;\t\thttps://github.com/cppfastio/fast_io"),
         .contact = tsc(u"Dwayne Bailey\t\t;\t\tfast_io"),
         .email = tsc(u"dwayne@translate.org.za;euloanty@live.com"),
         .tel = tsc(u""),
         .fax = tsc(u""),
         .language = tsc(u"South Ndebele"),
         .territory = tsc(u"South Africa"),
         .revision = tsc(u"0.3"),
         .date = tsc(u"2005-10-13")},
    .monetary = {.int_curr_symbol = tsc(u"ZAR "),
                 .currency_symbol = tsc(u"R"),
                 .mon_decimal_point = tsc(u"."),
                 .mon_thousands_sep = tsc(u","),
                 .mon_grouping = {numeric_grouping_storage, 1},
                 .positive_sign = tsc(u""),
                 .negative_sign = tsc(u"-"),
                 .int_frac_digits = 2,
                 .frac_digits = 2,
                 .p_cs_precedes = 1,
                 .p_sep_by_space = 0,
                 .n_cs_precedes = 1,
                 .n_sep_by_space = 0,
                 .p_sign_posn = 1,
                 .n_sign_posn = 1},
    .numeric = {.decimal_point = tsc(u"."), .thousands_sep = tsc(u","), .grouping = {numeric_grouping_storage, 1}},
    .time = {.abday = {tsc(u"Son"), tsc(u"Mvu"), tsc(u"Bil"), tsc(u"Tha"), tsc(u"Ne"), tsc(u"Hla"), tsc(u"Gqi")},
             .day = {tsc(u"uSonto"), tsc(u"uMvulo"), tsc(u"uLesibili"), tsc(u"lesithathu"), tsc(u"uLesine"),
                     tsc(u"ngoLesihlanu"), tsc(u"umGqibelo")},
             .abmon = {tsc(u"Jan"), tsc(u"Feb"), tsc(u"Mat"), tsc(u"Apr"), tsc(u"Mey"), tsc(u"Jun"), tsc(u"Jul"),
                       tsc(u"Arh"), tsc(u"Sep"), tsc(u"Okt"), tsc(u"Usi"), tsc(u"Dis")},
             .mon = {tsc(u"Janabari"), tsc(u"uFeberbari"), tsc(u"uMatjhi"), tsc(u"u-Apreli"), tsc(u"Meyi"),
                     tsc(u"Juni"), tsc(u"Julayi"), tsc(u"Arhostosi"), tsc(u"Septemba"), tsc(u"Oktoba"),
                     tsc(u"Usinyikhaba"), tsc(u"Disemba")},
             .d_t_fmt = tsc(u"%a %-e %b %Y %T %Z"),
             .d_fmt = tsc(u"%d//%m//%Y"),
             .t_fmt = tsc(u"%T"),
             .t_fmt_ampm = tsc(u""),
             .date_fmt = tsc(u"%a %b %-e %H:%M:%S %Z %Y"),
             .am_pm = {tsc(u""), tsc(u"")},
             .week = {7, 19971130, 1}},
    .messages = {.yesexpr = tsc(u"^[+1yY]"), .noexpr = tsc(u"^[-0nN]")},
    .paper = {.width = 210, .height = 297},
    .telephone = {.tel_int_fmt = tsc(u"+%c %a %l"),
                  .tel_dom_fmt = tsc(u"(%A) %l"),
                  .int_select = tsc(u"00"),
                  .int_prefix = tsc(u"27")},
    .name = {.name_fmt = tsc(u"%d%t%g%t%m%t%f")},
    .address = {.postal_fmt = tsc(u"%f%N%a%N%d%N%b%N%s %h %e %r%N%z %T%N%c%N"),
                .country_name = tsc(u"iSewula Afrika"),
                .country_post = tsc(u"ZA"),
                .country_ab2 = tsc(u"ZA"),
                .country_ab3 = tsc(u"ZAF"),
                .country_num = 710,
                .country_car = tsc(u"ZA"),
                .lang_name = tsc(u"isiNdebele"),
                .lang_ab = tsc(u"nr"),
                .lang_term = tsc(u"nbl"),
                .lang_lib = tsc(u"nbl")},
    .measurement = {.measurement = 1}};

inline constexpr u32lc_all u32lc_all_global{
    .identification =
        {.name = tsc(U"nr_ZA"),
         .encoding = tsc(FAST_IO_LOCALE_UENCODING),
         .title = tsc(U"Southern Ndebele locale for South Africa"),
         .source = tsc(U"Zuza Software Foundation (Translate.org.za)\t\t;\t\tfast_io"),
         .address =
             tsc(U"PO Box 28364, Sunnyside, 0132, South "
                 U"Africa\t\t;\t\thttps://gitee.com/qabeowjbtkwb/fast_io\t\t;\t\thttps://github.com/cppfastio/fast_io"),
         .contact = tsc(U"Dwayne Bailey\t\t;\t\tfast_io"),
         .email = tsc(U"dwayne@translate.org.za;euloanty@live.com"),
         .tel = tsc(U""),
         .fax = tsc(U""),
         .language = tsc(U"South Ndebele"),
         .territory = tsc(U"South Africa"),
         .revision = tsc(U"0.3"),
         .date = tsc(U"2005-10-13")},
    .monetary = {.int_curr_symbol = tsc(U"ZAR "),
                 .currency_symbol = tsc(U"R"),
                 .mon_decimal_point = tsc(U"."),
                 .mon_thousands_sep = tsc(U","),
                 .mon_grouping = {numeric_grouping_storage, 1},
                 .positive_sign = tsc(U""),
                 .negative_sign = tsc(U"-"),
                 .int_frac_digits = 2,
                 .frac_digits = 2,
                 .p_cs_precedes = 1,
                 .p_sep_by_space = 0,
                 .n_cs_precedes = 1,
                 .n_sep_by_space = 0,
                 .p_sign_posn = 1,
                 .n_sign_posn = 1},
    .numeric = {.decimal_point = tsc(U"."), .thousands_sep = tsc(U","), .grouping = {numeric_grouping_storage, 1}},
    .time = {.abday = {tsc(U"Son"), tsc(U"Mvu"), tsc(U"Bil"), tsc(U"Tha"), tsc(U"Ne"), tsc(U"Hla"), tsc(U"Gqi")},
             .day = {tsc(U"uSonto"), tsc(U"uMvulo"), tsc(U"uLesibili"), tsc(U"lesithathu"), tsc(U"uLesine"),
                     tsc(U"ngoLesihlanu"), tsc(U"umGqibelo")},
             .abmon = {tsc(U"Jan"), tsc(U"Feb"), tsc(U"Mat"), tsc(U"Apr"), tsc(U"Mey"), tsc(U"Jun"), tsc(U"Jul"),
                       tsc(U"Arh"), tsc(U"Sep"), tsc(U"Okt"), tsc(U"Usi"), tsc(U"Dis")},
             .mon = {tsc(U"Janabari"), tsc(U"uFeberbari"), tsc(U"uMatjhi"), tsc(U"u-Apreli"), tsc(U"Meyi"),
                     tsc(U"Juni"), tsc(U"Julayi"), tsc(U"Arhostosi"), tsc(U"Septemba"), tsc(U"Oktoba"),
                     tsc(U"Usinyikhaba"), tsc(U"Disemba")},
             .d_t_fmt = tsc(U"%a %-e %b %Y %T %Z"),
             .d_fmt = tsc(U"%d//%m//%Y"),
             .t_fmt = tsc(U"%T"),
             .t_fmt_ampm = tsc(U""),
             .date_fmt = tsc(U"%a %b %-e %H:%M:%S %Z %Y"),
             .am_pm = {tsc(U""), tsc(U"")},
             .week = {7, 19971130, 1}},
    .messages = {.yesexpr = tsc(U"^[+1yY]"), .noexpr = tsc(U"^[-0nN]")},
    .paper = {.width = 210, .height = 297},
    .telephone = {.tel_int_fmt = tsc(U"+%c %a %l"),
                  .tel_dom_fmt = tsc(U"(%A) %l"),
                  .int_select = tsc(U"00"),
                  .int_prefix = tsc(U"27")},
    .name = {.name_fmt = tsc(U"%d%t%g%t%m%t%f")},
    .address = {.postal_fmt = tsc(U"%f%N%a%N%d%N%b%N%s %h %e %r%N%z %T%N%c%N"),
                .country_name = tsc(U"iSewula Afrika"),
                .country_post = tsc(U"ZA"),
                .country_ab2 = tsc(U"ZA"),
                .country_ab3 = tsc(U"ZAF"),
                .country_num = 710,
                .country_car = tsc(U"ZA"),
                .lang_name = tsc(U"isiNdebele"),
                .lang_ab = tsc(U"nr"),
                .lang_term = tsc(U"nbl"),
                .lang_lib = tsc(U"nbl")},
    .measurement = {.measurement = 1}};

} // namespace
} // namespace fast_io_i18n

#include "../main.h"